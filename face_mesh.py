        ...
        if CONFIG["enable_face_detection"] and face_results and face_results.multi_face_landmarks:
            for face_landmarks in face_results.multi_face_landmarks:
                ...
                if CONFIG.get("show_face_mesh_in_box", True):
                    ...
                    # Enhanced scale factor for professional visualization
                    scale_factor = 2.2  # Optimized for technical detail
                    
                    # Professional face mesh rendering with depth-based coloring
                    if CONFIG["face_mesh_tesselation"]:
                        for connection in mp_face_mesh.FACEMESH_TESSELATION:
                            ...
                            # Calculate depth-based intensity (z-coordinate for realism)
                            depth_intensity = int(255 * (1 - min(start.z, end.z) * 2))
                            depth_intensity = max(50, min(255, depth_intensity))  # Clamp values
                            ...
                            # Professional cyan mesh with depth variation
                            cv2.line(frame, start_point, end_point, 
                                    (depth_intensity//3, depth_intensity, depth_intensity), 1)
                    
                    # Draw structural contours with enhanced visibility
                    for connection in mp_face_mesh.FACEMESH_CONTOURS:
                        ...
                        # Calculate visibility-based thickness
                        visibility = min(start.visibility, end.visibility) if hasattr(start, 'visibility') else 1.0
                        thickness = max(1, int(3 * visibility))
                        ...
                        # Professional green structural lines
                        cv2.line(frame, start_point, end_point, (0, 255, 128), thickness)
                    
                    # Enhanced iris rendering with realistic detail
                    for connection in mp_face_mesh.FACEMESH_IRISES:
                        ...
                        # Professional amber iris detail
                        cv2.line(frame, start_point, end_point, (0, 200, 255), 2)
